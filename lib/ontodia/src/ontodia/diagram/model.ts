...
export class DiagramModel extends Backbone.Model {
        ...
        const addClass = (cl: ClassTreeElement) => {
            ...
            if (!cl.icon) {
                if (cl.children.length === 0) {
                    cl.icon = require('../../../images/tree-leaf.png').toString();
                } else {
                    cl.icon = require('../../../images/tree-leaf-folder.png').toString();
                }
            }
    ...
    private initLinkSettings(linkSettings?: LinkTypeOptions[]) {
        if (linkSettings) {
            ...
            const indexedSettings = _.indexBy(linkSettings, 'id');
            _.each(this.linkTypes, (type, typeId) => {
                const settings = indexedSettings[typeId] || {isNew: true};
                type.set(_.defaults(settings, existingDefaults), {preventLoading: true});
            });
        } else {
        ...
        for (let i = 0; i < layoutData.cells.length; i++) {
            ...
            if (cell.type !== 'link') {
                ...
                if (template) {
                    ...
                    this.initializeElement(elementCellModel);
