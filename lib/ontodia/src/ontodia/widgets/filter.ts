...
class FilterElementView extends Backbone.View<Element> {
    ...
    public render(): FilterElementView {
        ...
        this.el.addEventListener('dragstart', (e) => {
            ...
            var elementIds: string[] = _.pluck(selectedElements, 'id');
            try {
            ...
            this.view.dragAndDropElements = _.indexBy(selectedElements, 'id');
    ...
    public updateView() {
        if (this.$div) {
            ...
            var backColor = this.view.getElementColor(template);
            var frontColor = this.model.get('selectedInFilter') ? d3.hcl(
                backColor.h, backColor.c, backColor.l * 1.2) : d3.hcl("white");
            this.$el.css({ background: backColor });
            this.$div.css({ background: frontColor });
            if (this.model.get('presentOnDiagram')) { this.$el.removeAttr('draggable'); }
            else { this.$el.attr('draggable', "true"); }
        }
    }
}
